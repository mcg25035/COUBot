...
def FTP(ID,password):
    ...
    def subthread_STOR():
        ...
        f.close()
        for i in a:
          client.send(i.encode("utf-8",errors='strict'))
        client.close()
    client = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    client.connect((host,port))
    client.recv(1024)
    client.send('USER cityofuniverse.1161_TW_#068'.encode("utf-8",errors='strict'))
    client.recv(1024)
    client.send('PASS COU006123'.encode("utf-8",errors='strict'))
    client.recv(1024)
    client.send('OPTS UTF8 ON'.encode("utf-8",errors='strict'))
    client.recv(1024)
    client.send('TYPE I'.encode("utf-8",errors='strict'))
    client.recv(1024)
